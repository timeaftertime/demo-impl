...
public class Grammer {
	...
	/**
	 * 消除直接左递归。 A -> Aa1|Aa2|...|b1|b2 ↓ A -> b1A'|b2A' A' -> a1A'|a2A'|...|ϵ
	 * 
	...
	private void initStartSymbol() {
				...
				for (NonTerminalSymbol s : copyOfNonTerminals()) {
					for (Production p : s.getProductions()) {
						if (p.getRights().contains(startSymbol)) {
							throw new CompilerException("语法开始符号不允许出现在产生式右式：" + p);
						}
					}
				}
				return;
		...
		throw new CompilerException("找不到语法开始符号: " + Keywords.CFG);
	}
	 ...
	 */
	public Symbol symbolOf(String code) throws CompilerException {
