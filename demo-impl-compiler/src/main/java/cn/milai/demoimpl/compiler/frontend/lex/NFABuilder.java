...
import cn.milai.demoimpl.compiler.frontend.lex.acceptor.CharAcceptor;
import cn.milai.demoimpl.compiler.frontend.lex.acceptor.ExcludeAcceptor;
import cn.milai.demoimpl.compiler.frontend.lex.acceptor.IncludeAcceptor;
...
public class NFABuilder {
	...
	private static NFAPair parseParenthesis(CharScanner scanner) {
		...
		throw new CompilerException("未匹配到右小括号");
	}
	...
	private static NFAPair applyRepeat(CharScanner scanner, NFAPair pair) {
		...
		if (scanner.now() == CharSets.OPEN_BRACE) {
			throw new CompilerException("暂不支持 {x,y} 表示重复的形式");
		}
		return pair;
	}
	...
	private static CharAcceptor parseBracket(CharScanner scanner) {
		...
		throw new CompilerException("未匹配到右中括号");
	}
