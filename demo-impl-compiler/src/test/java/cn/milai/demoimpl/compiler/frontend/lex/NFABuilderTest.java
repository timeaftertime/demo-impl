...
public class NFABuilderTest {
	...
	public void testLineNFA() {
		...
		s = addEmptyHead(s);
		// nfa =
		// head--ϵ-->s0--a-->s1--ϵ-->s2--b-->s3--ϵ-->s4--c-->s5--ϵ-->s6--d-->s7--ϵ-->s8--e-->s9
		for (int i = 0; i < 5; i++) {
	...
	public void testInputSet() {
		...
		// +---------------ϵ--------------↑
		Node s0 = NFABuilder
				.newNFA(new HashSet<>(Arrays.asList(new TokenDefinition("a[xy\\d][a-z]*", TEST_TOKEN_CODE1))));
		assertFalse(s0.isAccept());
		Node s1 = s0.next('a');
		assertNotNull(s1);
		assertFalse(s1.isAccept());
	...
	public void testComposeRepeatNFA() {
		...
		// +-----------------------------------------ϵ-----------------------------------------------+
		Node s0 = NFABuilder
				.newNFA(new HashSet<>(Arrays.asList(new TokenDefinition("a([bc]|d|e)*", TEST_TOKEN_CODE1))));
		assertFalse(s0.isAccept());
		Node s1 = s0.next('a');
		assertNotNull(s1);
		assertFalse(s1.isAccept());
	...
	public void testMultipleRe() {
		// +-->s0---a---->s1
		// s4--ϵ--+-->s2--[cd]-->s3
		Node s4 = NFABuilder.newNFA(new HashSet<>(Arrays.asList(new TokenDefinition("a", TEST_TOKEN_CODE1),
