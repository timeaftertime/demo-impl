...
import cn.milai.demoimpl.compiler.frontend.lex.LexTestUtils;
...
public class ParserTest {
	...
	public void testParseIf() {
		...
		Node root = new Parser(grammer)
				.parse(LexTestUtils.LEXER.lex(new CharScanner("if(isTest) {   doSomethingTest() ;   }")));
	...
	public void testParseStmds() {
		/**
		 * 由于构造语法时中间非终结符的生成，符号名与实际有些不同，但结构是一样的 CFG ↓ Stmds1
		 * +---------+-----------------+ ↓ ↓ Stmd1 Stmds2 +-----+---+--+---+
		 * +------------------+ ↓ ↓ ↓ ↓ ↓ ↓ ↓ while ( Expr ) Block Stmd4 Stmds6 ↓ ↓
		 * +-+-+-+ +--------------------------+ ID +---+ ↓ ↓ ↓ ↓ Stmd5 Stmds7 ↓ ↓ ID ( )
		 * ; ↓ +----------+ { Stmds3 ; Stmd6 Stmd' +----------------+
		 * +-------------------+ ↓ Stmds4 ↓ ↓ ↓ ↓ ↓ ↓ ϵ +------+-------+ } if ( Expr )
		 * Block Stmd2 Stmds5 ↓ ↓ +----+---+ +------------+ ID Stmd ↓ ↓ ↓ ↓ Stmd3 Stmd'
		 * ↓ ID ( ) ; +----+------+ ↓ ; ↓ ↓ ↓ ↓ ϵ ID ( ) ;
		 */
		String FILE = "/parsing/testParseStmds.txt";
		Grammer grammer = GrammerReader.parseGrammer(ParserTest.class.getResourceAsStream(FILE));
		Node root = new Parser(grammer).parse(LexTestUtils.LEXER
				.lex(new CharScanner("while(isTest) {  test1();test2();   }    doSomething();  ; if(isProd) ;")));
		List<Node> rootChildren = root.getChildren();
		assertEquals(1, rootChildren.size());
